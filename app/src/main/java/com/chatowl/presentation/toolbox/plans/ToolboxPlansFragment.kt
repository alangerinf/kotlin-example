...
import kotlinx.android.synthetic.main.fragment_toolbox_plans.*
import kotlinx.android.synthetic.main.fragment_toolbox_plans.fragment_toolbox_recycler_view
import kotlinx.android.synthetic.main.fragment_toolbox_plans.fragment_toolbox_scrollview

class ToolboxPlansFragment: ViewModelFragment<ToolboxPlansViewModel>(ToolboxPlansViewModel::class.java) {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_toolbox_plans, container, false)
    }

    override fun setViewModel(): ToolboxPlansViewModel {
        ...
        return ViewModelProvider(this, ToolboxPlansViewModel.Factory(application, repository)).get(
            ToolboxPlansViewModel::class.java
        )
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        fragment_toolbox_plans_text_view_filter.setOnClickListener {
            animateFilter()
        }

        fragment_toolbox_plans_image_button_search.setOnClickListener {
            viewModel.showSearchField()
        }

        fragment_toolbox_plans_edit_text_search_field.addTextChangedListener(afterTextChanged = {
            viewModel.onFilterSearchChanged(it.toString())
        })

        fragment_toolbox_plans_edit_text_search_field.setOnFocusChangeListener { _, b ->
            if(fragment_toolbox_plans_edit_text_search_field.text.toString().isEmpty() && !b) {
                viewModel.hideSearchField()
            }
        }

        fragment_toolbox_plans_text_view_filter_media_audio.setOnClickListener {
            viewModel.onFilterAudioClicked()
        }

        fragment_toolbox_plans_text_view_filter_media_image.setOnClickListener {
            viewModel.onFilterImageClicked()
        }

        fragment_toolbox_plans_text_view_filter_media_quote.setOnClickListener {
            viewModel.onFilterQuoteClicked()
        }

        fragment_toolbox_plans_text_view_filter_media_video.setOnClickListener {
            viewModel.onFilterVideoClicked()
        }

        fragment_toolbox_plans_text_view_filter_length_short.setOnClickListener {
            viewModel.onFilterShortClicked()
        }

        fragment_toolbox_plans_text_view_filter_length_medium.setOnClickListener {
            viewModel.onFilterMediumClicked()
        }

        fragment_toolbox_plans_text_view_filter_length_long.setOnClickListener {
    ...
    override fun addObservers() {
        ...
        viewModel.isLoadingPlanOrCategory.observe(viewLifecycleOwner, { isLoadingPlanOrCategory ->
            fragment_toolbox_plans_linear_progress_indicator.visibility =
                if (isLoadingPlanOrCategory) View.VISIBLE else View.GONE
        })
        viewModel.isLoadingAndEmpty.observe(viewLifecycleOwner, { isLoading ->
            if (isLoading) {
                fragment_toolbox_plans_progress_bar.visibility = View.VISIBLE
                fragment_toolbox_scrollview.visibility = View.GONE
            } else {
                fragment_toolbox_plans_progress_bar.visibility = View.GONE
                fragment_toolbox_scrollview.visibility = View.VISIBLE
            }
        })
        viewModel.filteredGroups.observe(viewLifecycleOwner, { groups ->
            fragment_toolbox_plans_layout_no_results.visibility =
                if (groups.isEmpty()) View.VISIBLE else View.GONE
            groupAdapter.submitList(groups)
        })
        viewModel.showSearchField.observe(viewLifecycleOwner, { showSearch ->
            if(showSearch) {
                fragment_toolbox_plans_layout_search.visibility = View.VISIBLE
                fragment_toolbox_plans_image_button_search.visibility = View.GONE
            } else {
                fragment_toolbox_plans_layout_search.visibility = View.GONE
                fragment_toolbox_plans_image_button_search.visibility = View.VISIBLE
            }
        })
        
        viewModel.audioFilter.observe(viewLifecycleOwner, {
            setFilterSelected(fragment_toolbox_plans_text_view_filter_media_audio, it)
        })
        viewModel.imageFilter.observe(viewLifecycleOwner, {
            setFilterSelected(fragment_toolbox_plans_text_view_filter_media_image, it)
        })
        viewModel.quoteFilter.observe(viewLifecycleOwner, {
            setFilterSelected(fragment_toolbox_plans_text_view_filter_media_quote, it)
        })
        viewModel.videoFilter.observe(viewLifecycleOwner, {
            setFilterSelected(fragment_toolbox_plans_text_view_filter_media_video, it)
        })
        viewModel.lengthShortFilter.observe(viewLifecycleOwner, {
            setFilterSelected(fragment_toolbox_plans_text_view_filter_length_short, it)
        })
        viewModel.lengthMediumFilter.observe(viewLifecycleOwner, {
            setFilterSelected(fragment_toolbox_plans_text_view_filter_length_medium, it)
        })
        viewModel.lengthLongFilter.observe(viewLifecycleOwner, {
            setFilterSelected(fragment_toolbox_plans_text_view_filter_length_long, it)
        })

        viewModel.isAllFiltersEnabled.observe(viewLifecycleOwner, {
            fragment_toolbox_plans_text_view_filter.visibility = if (it) View.VISIBLE else View.GONE
        })

        viewModel.isMediaFilterEnabled.observe(viewLifecycleOwner, {
            fragment_toolbox_plans_text_view_filter_media.visibility = if (it) View.VISIBLE else View.GONE
        })
        viewModel.isAudioFilterEnabled.observe(viewLifecycleOwner, {
            fragment_toolbox_plans_text_view_filter_media_audio.visibility = if (it) View.VISIBLE else View.GONE
        })
        viewModel.isImageFilterEnabled.observe(viewLifecycleOwner, {
            fragment_toolbox_plans_text_view_filter_media_image.visibility = if (it) View.VISIBLE else View.GONE
        })
        viewModel.isQuoteFilterEnabled.observe(viewLifecycleOwner, {
            fragment_toolbox_plans_text_view_filter_media_quote.visibility = if (it) View.VISIBLE else View.GONE
        })
        viewModel.isVideoFilterEnabled.observe(viewLifecycleOwner, {
            fragment_toolbox_plans_text_view_filter_media_video.visibility = if (it) View.VISIBLE else View.GONE
        })
        viewModel.isDurationFilterEnabled.observe(viewLifecycleOwner, {
            fragment_toolbox_plans_text_view_filter_length.visibility = if (it) View.VISIBLE else View.GONE
        })
        viewModel.isShortFilterEnabled.observe(viewLifecycleOwner, {
            fragment_toolbox_plans_text_view_filter_length_short.visibility = if (it) View.VISIBLE else View.GONE
        })
        viewModel.isMediumFilterEnabled.observe(viewLifecycleOwner, {
            fragment_toolbox_plans_text_view_filter_length_medium.visibility = if (it) View.VISIBLE else View.GONE
        })
        viewModel.isLongFilterEnabled.observe(viewLifecycleOwner, {
            fragment_toolbox_plans_text_view_filter_length_long.visibility = if (it) View.VISIBLE else View.GONE
        })
    ...
    private fun animateFilter() {
        if (fragment_toolbox_plans_layout_filters.visibility == View.VISIBLE) {
            fragment_toolbox_plans_layout_filters.startAnimation(filterClose)
        } else {
            fragment_toolbox_plans_layout_filters.startAnimation(filterOpen)
        }
    }
    ...
    private fun loadAnimations(context: Context) {
        ...
        filterOpen.setAnimationListener(object : Animation.AnimationListener {
            override fun onAnimationStart(p0: Animation?) {
                fragment_toolbox_plans_layout_filters.visibility = View.VISIBLE
            }
        ...
        filterClose.setAnimationListener(object : Animation.AnimationListener {
            ...
            override fun onAnimationEnd(p0: Animation?) {
                fragment_toolbox_plans_layout_filters.visibility = View.GONE
            }
