...
class QuoteExerciseViewModel(
    application: Application,
    args: QuoteExerciseFragmentArgs,
    ...
    class Factory(
        ...
        private val args: QuoteExerciseFragmentArgs,
        private val toolboxRepository: ToolboxRepository
    ) : ViewModelProvider.Factory {
        override fun <T : ViewModel?> create(modelClass: Class<T>): T {
            return QuoteExerciseViewModel(application, args, toolboxRepository) as T
        }
    }
...
//    }
    ...
    private fun navigateToExercise(exercise: ToolboxExercise) {
        when (exercise.tool.toolSubtype) {
            ToolSubtype.VIDEO.name.lowerCase(), ToolSubtype.AUDIO.name.lowerCase() -> {
                _navigate.value =
                    QuoteExerciseFragmentDirections.actionGlobalMediaExercise(exercise)
            ...
            ToolSubtype.IMAGE.name.lowerCase() -> {
                _navigate.value =
                    QuoteExerciseFragmentDirections.actionGlobalImageExercise(exercise)
                        .setHeader(header.value)
    ...
    private fun saveAsFavorite(add: Boolean, toolId: Int) {
        viewModelScope.fetch({
            ...
            it.printStackTrace()
            val eventTypeString =
                if (add) OfflineEventType.EXERCISE_SAVE_FAVORITE else OfflineEventType.EXERCISE_REMOVE_FAVORITE
